...
import { requiredImports_manual_trigger } from "./requiredImports";

const loadSceneFromSceneData_manual_trigger = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_manual_trigger.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_manual_trigger.operations) {
  ...
  const sceneName = "manual_trigger";
...
export { Poly, loadSceneFromSceneData_manual_trigger };
